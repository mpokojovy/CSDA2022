...
y.labels   = c(bquote(e[mu]), bquote(e[Sigma]^1), bquote(e[Sigma]^2),
...
for (rho.mult in rho.mult.array)
{
  ...
  for (i.stat.type in 1:length(stat.types))
  {
    ...
    grDevices::pdf(file = paste(dir.name, "/", file.name, ".pdf", sep = ""), width = 8, height = 8)
    ...
    lines(n.array, ppmcd.err,      lty = 1, lwd = 1, col = "black")
    lines(n.array, ppmcd.std.err,  lty = 1, lwd = 2, col = "black")
    lines(n.array, detmcd.err,     lty = 2, lwd = 1, col = "red")
    lines(n.array, detmcd.std.err, lty = 2, lwd = 2, col = "red")
    lines(n.array, fastmcd.err,    lty = 4, lwd = 2, col = "blue")
