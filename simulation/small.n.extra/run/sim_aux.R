...
getSummary <- function(mcd, G, m) {
  ...
  WS.names = sapply(1:7, function(i) paste("WS", i, sep = ""))
  ...
  if (tolower(mcd$nsamp) == "pp.plus") {
    WS = rep(0, 7)
    WS[mcd$iBest] = 1.0/length(mcd$iBest)
  } else if (tolower(mcd$nsamp) == "deterministic") {
    WS = c(rep(0, 6), NA)
    WS[mcd$iBest] = 1.0/length(mcd$iBest)
  } else {
    WS = rep(NA, 7)
...
applyMCD <- function(mcd.type = "PP", p, n, eps = 0.5, rho.mult = 0.75, r = NA, delta = NA, psi = NA, standardize = TRUE) {
  ...
  if (standardize) {
    ...
    mcd = PPcovMcd(mah.std$z.scores, nsamp = mcd.type, wgtFUN = wgtFUN)
    ...
    if (is.element(tolower(mcd.type), c("pp", "pp.plus"))) {
      mcd = PPcovMcd(samp$dat, nsamp = mcd.type, wgtFUN = wgtFUN, PP.standardization = "diagonal")
    } else
...
doSimulation <- function(mcd.type = "PP", nsim = 100, p, n, eps = 0.5, rho.mult = 0.75, r = NA, delta = NA, psi = NA, standardize = TRUE) {
  if (parallel.flag) {
    ...
    sim.resp = foreach(ind = 1:nsim, .inorder = FALSE, .packages = c("robustbase", "PPcovMcd"),
                       .export = c("drawSample", "getSummary", "applyMCD"), .combine = "rbind") %do%
                       applyMCD(mcd.type, p, n, eps, rho.mult, r, delta, psi, standardize)
  ...
  WS.avg = sapply(1:7, function(i) mean(sim.resp[, paste("WS", i, sep = "")]))
  #WS.avg = sapply(1:7, function(i) mean(sim.resp[9 + i, ]))
