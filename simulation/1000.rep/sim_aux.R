# NOTE THE PURITY IS BASED ON THE RAW BEST SET HERE!
...
getSummary <- function(mcd, G, m) {
  ...
  breakdown = if (m == 0L) 0 else (sum(wt[(n - m + 1):n]) > 0)
  impurity  = if (m == 0L) 0 else sum(wt[(n - m + 1):n])/max(1, sum(wt)) # outliers are assumed to come at the end of the sample
  n.outlier = if (m == 0L) 0 else sum(wt[(n - m + 1):n])
  bulk.size = sum(wt)
