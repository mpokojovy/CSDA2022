...
if (parallel.flag) {
  for (n in n.in:n.max) {
    ...
    wgtFUN.ppmcd   <- function(dist) (dist < ppmcd.quantiles.rnd.raw.0.9[n - n.in + 1]*median(dist)/ppmcd.quantiles.rnd.raw.0.5[n - n.in + 1])
    wgtFUN.fastmcd <- function(dist) (dist < fastmcd.quantiles.rnd.raw.0.9[n - n.in + 1]*median(dist)/fastmcd.quantiles.rnd.raw.0.5[n - n.in + 1])
    wgtFUN.detmcd  <- function(dist) (dist < detmcd.quantiles.rnd.raw.0.9[n - n.in + 1]*median(dist)/detmcd.quantiles.rnd.raw.0.5[n - n.in + 1])
  ...
  for (i in 1:nrep) {
    ...
    for (n in n.in:n.max) {
      ...
      wgtFUN <- function(dist) (dist < ppmcd.quantiles.max.raw.0.9[n - n.in + 1]*median(dist)/ppmcd.quantiles.max.raw.0.5[n - n.in + 1])
      ppmcd   = PPcovMcd(mah.std$z.scores, nsamp = "PP",            wgtFUN = wgtFUN)
      
      wgtFUN <- function(dist) (dist < fastmcd.quantiles.max.raw.0.9[n - n.in + 1]*median(dist)/fastmcd.quantiles.max.raw.0.5[n - n.in + 1])
      fastmcd = PPcovMcd(mah.std$z.scores, nsamp = 500,             wgtFUN = wgtFUN)
      
      wgtFUN <- function(dist) (dist < detmcd.quantiles.max.raw.0.9[n - n.in + 1]*median(dist)/detmcd.quantiles.max.raw.0.5[n - n.in + 1])
...
ppmcd.logdet.rew   = -smooth.monotonic(n.in:n.max, -(logdet(cov(mah.standard(dataset)$z.scores)) - colMeans(ppmcd.logdet.rew)))
fastmcd.logdet.rew = -smooth.monotonic(n.in:n.max, -(logdet(cov(mah.standard(dataset)$z.scores)) - colMeans(fastmcd.logdet.rew)))
detmcd.logdet.rew  = -smooth.monotonic(n.in:n.max, -(logdet(cov(mah.standard(dataset)$z.scores)) - colMeans(detmcd.logdet.rew)))
...
plot(n.in:n.max, ppmcd.quantiles.max.rew.0.95/exp(ppmcd.logdet.rew/p), type = "l", lty = 1, col = "black", lwd = 2, xlab = "Sample size n", ylab = "Sq. Mahalanobis distance 95-th quantile",
     xlim = c(n.in, n.max), ylim = c(0, 1000), main = "PP MCD")
...
plot(n.in:n.max, fastmcd.quantiles.max.rew.0.95/exp(fastmcd.logdet.rew/p), type = "l", lty = 2, col = "blue", lwd = 2, xlab = "Sample size n", ylab = "Sq. Mahalanobis distance 95-th quantile",
     xlim = c(n.in, n.max), ylim = c(0, 1000), main = "FastMCD")
...
plot(n.in:n.max, detmcd.quantiles.max.rew.0.95/exp(detmcd.logdet.rew/p), type = "l", lty = 4, col = "red", lwd = 2, xlab = "Sample size n", ylab = "Sq. Mahalanobis distance 95-th quantile",
     xlim = c(n.in, n.max), ylim = c(0, 1000), main = "DetMCD")
